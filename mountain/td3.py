...
#####################  hyper parameters  ####################
# choose env
ENV_ID = 'MountainCarContinuous-v0'  # environment id
...
class ReplayBuffer:
    ...
    def add(self, state, action, next_state, done):
        self.temp.append([state, action, next_state, done])
    def addReward(self, reward):
        for x in self.temp:
            self.push(x[0], x[1], reward, x[2], x[3])
        self.temp = [ ]
...
def rewardCalculator(trainer, swingMode, lastRightLeft, nextState, reward):
    print(swingMode, nextState)
    if swingMode == 0:
        if nextState[1] < 0:
            swingMode = -1
        else:
            swingMode = +1
        print("if1", reward)
        return swingMode, lastRightLeft, reward
    elif swingMode == +1 and nextState[1] < 0:
        # found right point
        swingMode = -1
        rightPoint = nextState[0] + 0.52
        leftPoint = lastRightLeft
        if leftPoint < 0:
            if rightPoint <= 0:
                reward = rightPoint / leftPoint * 2 + 8
            elif rightPoint < abs(leftPoint):
                reward = (abs(leftPoint) - rightPoint) / abs(leftPoint) * 5 + 3
            elif rightPoint < abs(leftPoint * 3 / 2):
                span = (abs(leftPoint * 3 / 2) - abs(leftPoint))
                reward = (span - (rightPoint - abs(leftPoint))) / span * 3
            else:
                reward = 0
        else:
            reward = 0
        reward = - reward
        # print(1, rightPoint, lastRightLeft, reward)
        trainer.addReward(reward)
        lastRightLeft = rightPoint
        print("if2", reward)
        return swingMode, lastRightLeft, reward
    elif swingMode == -1 and nextState[1] > 0:
        # found left point
        swingMode = +1
        leftPoint = nextState[0] + 0.52
        rightPoint = lastRightLeft
        if rightPoint > 0:
            if leftPoint >= 0:
                reward = leftPoint / rightPoint * 5 + 5
            elif abs(leftPoint) <= rightPoint * 3 / 2:
                reward = (rightPoint * 3 / 2 - abs(leftPoint)) / abs(rightPoint * 3 / 2) * 5
            else:
                reward = (abs(leftPoint) - rightPoint * 3 / 2) / (1.2 - 0.52 - rightPoint * 3 / 2) * 10
        else:
            reward = 10
        reward = - reward
        # print(-1, leftPoint, lastRightLeft, reward)
        trainer.addReward(reward)
        lastRightLeft = leftPoint
        print("if3", reward)
        return swingMode, lastRightLeft, reward
    
    # print(swingMode, lastRightLeft, reward)
    return swingMode, lastRightLeft, reward
    ...
    if trainT:
        ...
        for episode in range(TRAIN_EPISODES):
            ...
            for step in range(MAX_STEPS):
                ...
                replay_buffer.add(state, action, next_state, done)
                swingMode, lastRightLeft, reward = rewardCalculator(replay_buffer, swingMode, lastRightLeft, next_state, reward)
                state = next_state
            ...
            file1 = open("td3.txt", "a")
            file1.write('{}, {}, {}, {}'.format(episode_reward, episodeStep, totalStep, episode, t0 - time.time()))
            file1.write("\n")
            file1.close()
